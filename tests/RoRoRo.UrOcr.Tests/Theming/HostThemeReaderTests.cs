...
namespace RoRoRo.UrOcr.Tests.Theming;
...
public class HostThemeReaderTests
{
    ...
    {
        var noSettings = Path.Combine(Path.GetTempPath(), "urocr-tests", Guid.NewGuid().ToString("N"));
    ...
    {
        var dir = Path.Combine(Path.GetTempPath(), "urocr-tests", Guid.NewGuid().ToString("N"));
