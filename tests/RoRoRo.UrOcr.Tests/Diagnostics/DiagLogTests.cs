...
namespace RoRoRo.UrOcr.Tests.Diagnostics;
...
public class DiagLogTests : IDisposable
{
    ...
    public DiagLogTests()
    {
        _dir = Path.Combine(Path.GetTempPath(), "urocr-diaglog-" + Guid.NewGuid().ToString("N"));
    ...
    public void Write_UncreatableDirectory_DisablesForSessionWithoutThrowing()
    {
        ...
        var blocker = Path.Combine(Path.GetTempPath(), "urocr-blocker-" + Guid.NewGuid().ToString("N"));
